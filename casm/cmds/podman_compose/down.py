...
from ...utils import podman_compose_down
from .service import add_opt_services
from .service import filter_services


@add_command("down", help="Tear down services")
def add_cmd_down(_arg: argp):
    add_opt_services(_arg)


@run_command(add_cmd_down)
def run_cmd_down(cmds: commands) -> int:
    assemble: assemble_file = cmds.args.assemble_file
    assert isinstance(assemble, assemble_file)
    services: List[str] = filter_services(assemble, cmds.args.services)
    return podman_compose_down(assemble.compose_file, *services)
